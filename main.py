...
class MainGUI:
    ...
    def InitMain(self):
        ...
        #프레임 구분을 위해 색칠
        frame1 = Frame(self.window, width=800, height=100, bg='gray')  # 상단 프레임 생성
        frame1.place(x=0,y=0)  # 상단에 프레임 배치
        label = Label(frame1, text="우리 동네 병원 찾기", font=self.TempFont, bg="gray")  # 라벨 생성
        label.place(relx=0.5, rely=0.5, anchor=CENTER)

        frame2 = Frame(self.window, width=400, height=500, bg='blue')  # 왼쪽 프레임 생성
        frame2.place(x=0, y=100)  # 왼쪽에 프레임 배치
        # 병원 이미지(로고)


        frame3 = Frame(self.window, width=400, height=500, bg='green')  # 오른쪽 프레임 생성
    ...
    def create_nameSearch_frame(self):
        ...
        frame_imformation = Frame(frame_left, width=500, height=550, relief='solid', borderwidth=2, bg='red')
        frame_imformation.place(x=50, y=100)
